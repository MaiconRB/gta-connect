...
- **Testes automatizados**: 173 testes no backend (xUnit, Domain + Application), 1 no frontend — cobrindo entidades de domínio, validators, services com mocks; features de conexões/avaliação e notificações também verificadas ponta a ponta (curl contra o banco real + Playwright com dois usuários no navegador, incluindo tempo real via SignalR sem reload).

### Em andamento / próximo (nesta ordem, combinada com o usuário)

1. A definir — candidatos: painel de revisão de denúncias, "seguir" jogadores (feed hoje é só público/global — a entidade `Connection` já existe e poderia alimentar isso), "online agora" (ver seção 12), nome definitivo do app.
