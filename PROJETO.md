...
Plano pós-MVP em andamento (plano completo salvo em `C:\Users\mrbcp\.claude\plans\vamos-come-ar-a-desenvolver-fluttering-dolphin.md`): reputação de confiabilidade + score de compatibilidade (Fase 1, concluída em 22/08) → design system e acessibilidade (Fase 4, concluída em 01/09, adiantada na ordem por prioridade de UX) → próximo: IA própria treinada em cima dos dados do app (Fase 2) e/ou dar vida ao app com seed/onboarding (Fase 3) → produção/infra (nome do app, Docker, CI/CD, Kubernetes local, Postgres) (Fase 5) → dívida técnica (Fase 6).
