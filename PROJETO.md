...
- **Testes automatizados**: 188 testes no backend (xUnit, Domain + Application), 1 no frontend — cobrindo entidades de domínio, validators, services com mocks; features de conexões/avaliação, notificações, feed filtrado e painel de moderação também verificadas ponta a ponta (curl contra o banco real + Playwright com dois usuários no navegador, incluindo tempo real via SignalR sem reload e fluxo completo de banimento/desbanimento).

### Em andamento / próximo (nesta ordem, combinada com o usuário)

1. A definir — candidatos: "online agora" (indicador de presença, ver seção 12), nome definitivo do app.
